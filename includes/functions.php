<?php
	...
	function mysql_prep( $value ) {
		...
		if( $new_enough_php ) { // PHP v4.3.0 or higher
			...
			// if magic quotes aren't alreay on then add slashes manually
	...
	function get_all_subjects_public() {
		global $connection;
		$query = "SELECT * 
				FROM Subjects ";
	...
	function get_pages_for_subject_public($subject_id) {
		global $connection;
		$query = "SELECT * 
				FROM pages ";
	...
	function public_navigation($sel_subject, $sel_page) {
		$output = "<ul class=\"Subjects\">";
		$subject_set = get_all_subjects_public();
		while ($subject = mysql_fetch_array($subject_set)) {
			$output .= "<li";
			if ($subject["id"] == $sel_subject['id']) { $output .= " class=\"selected\""; }
			...
			if ($subject["id"] == $sel_subject['id']) {	
				$page_set = get_pages_for_subject_public($subject["id"]);
				$output .= "<ul class=\"pages\">";
				while ($page = mysql_fetch_array($page_set)) {
					$output .= "<li";
					if ($page["id"] == $sel_page['id']) { $output .= " class=\"selected\""; }
